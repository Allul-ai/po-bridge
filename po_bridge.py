...
Variables d'environnement Render :
    ...
    PO_RATTRAPAGE_MULT = multiplicateur du rattrapage, defaut 2.25 (plafond 10% du solde)
...
RATTRAPAGE_CAP_PCT = 10.0        # la mise de rattrapage ne depasse jamais 10% du solde
...
def _execute_trade(sig):
    ...
    if RATTRAPAGE_ON and _RATTRAPAGE["arme"]:
        ...
        est_rattrapage = True
        pct = min(TRADE_PCT * RATTRAPAGE_MULT, RATTRAPAGE_CAP_PCT)
    amount = round(max(TRADE_MIN, balance * pct / 100.0), 2)
    ...
    return (f"\u2705 Ordre place : {sens} {sig['asset']}{tag}\n"
            f"Mise : {amount}$ ({round(pct, 2)}% de {round(balance, 2)}$) \u00b7 "
