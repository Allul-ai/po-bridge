...
def _fetch_direct(asset, tf):
    """Demande l'historique via le canal loadHistoryPeriod (requete explicite
    avec actif + periode + heure de fin). Contrairement a change_symbol, le
    serveur y repond a chaque demande, pour n'importe quel actif/timeframe.
    Renvoie ~150 bougies sous forme de liste de dicts {time,open,high,low,close}."""
    api = API.api
    try:
        end = int(API.get_server_timestamp() or time.time())
    except Exception:
        end = int(time.time())
    api.history_data = None
    api.getcandles(asset, tf, end)          # envoie loadHistoryPeriod
...
def _call_get_candles(asset, tf):
    """Recuperation directe, avec garde-fou temps global."""
    return _with_timeout(lambda: _fetch_direct(asset, tf),
                         25, f"loadHistory({asset},{tf})")
...
def maybe_alert(asset, tf):
    ...
    # meme direction -> re-alerte seulement apres TG_COOLDOWN (anti-spam :
    # un signal qui persiste 10 bougies ne doit pas envoyer 10 messages)
