...
def _call_get_candles(asset, tf):
    """Essaie plusieurs signatures selon la version de la librairie."""
    last_exc = None
    for args in ((asset, tf, NB_CANDLES),
                 (asset, tf),
                 (asset, tf, NB_CANDLES, time.time()),
                 (asset, tf, time.time(), NB_CANDLES)):
        try:
            df = API.get_candles(*args)
