...
def _merge_m1(asset, data):
    ...
    if st:
        try:
            ...
            if ref > 0 and abs(med / ref - 1) > 0.03:   # >3% d'ecart : impossible en 1 page M1
...
def fetch_asset(asset):
    ...
    try:
        ...
        if data is None:
            log.warning("%s: aucune reponse du serveur (12s) — flux possiblement fige", asset)
...
def _execute_trade(sig):
    """Place l'ordre sur Pocket Option. Renvoie un message de resultat."""
    today = _paris_date()
    ...
    if _TRADES_JOUR["count"] >= TRADE_MAX_JOUR:
        return (f"\U0001F6D1 Limite atteinte : {TRADE_MAX_JOUR} trades aujourd'hui. "
                "Rien n'a ete place. (PO_MAX_TRADES_JOUR pour ajuster)")
    if not CONNECTED["ok"] or API is None:
        return "\u274C Pont deconnecte de Pocket Option — rien n'a ete place."
    try:
        balance = API.get_balance()
    except Exception:
        balance = None
    if not balance or balance <= 0:
        return "\u274C Solde indisponible pour calculer les 2% — rien n'a ete place. Reessaie dans une minute."
    ...
    if not API_LOCK.acquire(timeout=30):
        return "\u274C Pont occupe — rien n'a ete place. Reessaie."
    try:
        ok, oid = _with_timeout(
            lambda: API.buy(amount, sig["asset"], action, exp_sec),
            70, f"buy({sig['asset']})")
    except Exception as e:
        return f"\u274C Erreur pendant l'ordre : {e}. Verifie sur PO si une position est ouverte avant de retenter."
    finally:
        API_LOCK.release()
    if not ok:
        return ("\u274C Ordre refuse par Pocket Option — rien n'a ete place. "
                "(solde insuffisant, actif ferme, ou montant hors limites)")
    _TRADES_JOUR["count"] += 1
    sens = "\U0001F7E2 ACHAT" if sig["dir"] == "buy" else "\U0001F534 VENTE"
    return (f"\u2705 Ordre place : {sens} {sig['asset']}\n"
            f"Mise : {amount}$ ({TRADE_PCT}% de {round(balance, 2)}$) \u00b7 "
            f"Expiration {sig['exp']} min\n"
            f"Trade {_TRADES_JOUR['count']}/{TRADE_MAX_JOUR} du jour \u00b7 id {oid}")
...
def telegram_poller():
    ...
    log.info("Ecoute Telegram active (execution en un tap, %s%% du solde, "
             "max %d trades/jour)", TRADE_PCT, TRADE_MAX_JOUR)
    while True:
        try:
